...
from flow.flow_utils import FlowModelFractureSegments
...
### flow
cond_c=PropertyMap(np.zeros(me.nelts,dtype=int),np.array([cond_rock]))  #uniform hydraulic conductivity
stor_c=PropertyMap(np.zeros(me.nelts,dtype=int),np.array([stor_rock]))  #uniform hydraulic storage
flow_properties={"Conductivity": cond_c,"Storage":stor_c}
constPerm=ConstantPerm(flow_properties,me.nelts)   # instantiate a constant permeability/storage model

# injection under constant pressure
the_inj=Injection(np.array([0.,0.]),np.array([[0.,dpcenter]]),"Pressure")
...
res_a_tol = 1.e-4*max(sigmap_o*me.nelts,1e3) # absolute convergence tolerance on mechanical residualts.
...
jac_solve_options=IterativeLinearSolve_options(max_iterations=200,absolute_tolerance=0.,
...
ax.plot(colPts,solp,'r-')
ax.plot(coor1D,res[jj].pressure,'b-')
plt.xlabel("x (M)")

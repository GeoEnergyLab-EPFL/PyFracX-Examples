...
f_d = 0.0
# injection rate
Qinj = 5.0e-6  #  m^2/s injected fluid rate in the total fracture
...
L_s = (
    ...
    * k_frac
    * (np.sqrt(4 * np.pi))
    / (f_p * Qinj * fluid_visc / wh_o)
)
t_s = L_s**2 / (4.0 * alpha_h)  # time of shear activation
#
L_op = L_s / One_minus_S
t_op = L_op**2 / 4.0 / alpha_h  # time of opening activation
...
in_situ_tractions = np.full(
    (Nelts, 2), [-tau_o, -sig_o_p]
)  # positive stress in traction !

# initial DDs.
DDs_initial = 0.0 * in_situ_tractions
...
elastic_m = Elasticity(
    kernel, elas_properties, max_leaf_size=32, eta=3.0, eps_aca=1.0e-5
)
# hmat creation
...
hyd_width = PropertyMap(
    np.zeros(me.nelts, dtype=int), np.array([wh_o])
)  # uniform hydraulic width
comp_c = PropertyMap(np.zeros(me.nelts, dtype=int), np.array([c_f]))  #
flow_properties = {
    "Fluid viscosity": fluid_visc,
    "Initial hydraulic width": hyd_width,
    "Compressibility": comp_c,
}
...
sol0 = HMFSolution(
    time=0.0,
    effective_tractions=effective_tractions_0.flatten(),
    pressure=P_f_nodes,
    DDs=DDs_initial.flatten(),
    DDs_plastic=DDs_initial.flatten(),
    pressure_rate=0.0 * P_f_nodes,
    DDs_rate=0.0 * effective_tractions_0.flatten(),
    Internal_variables=np.zeros(0),
    res_mech=0.0 * effective_tractions_0.flatten(),
    res_flow=0.0 * P_f_nodes,
)
...
my_simul.setupSimulation(
    sol0, tend, dt=dt_ini, maxSteps=maxSteps, saveEveryNSteps=1, log_level="INFO"
)

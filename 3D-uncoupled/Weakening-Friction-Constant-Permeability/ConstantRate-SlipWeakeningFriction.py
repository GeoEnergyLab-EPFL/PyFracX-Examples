...
with pygmsh.geo.Geometry() as geom:
    ...
    bottom_point = geom.add_point([0.0, -Lx_ext, 0.0], out_res)
    left_point = geom.add_point([-Lx_ext, 0.0, 0.0], out_res)
...
pressure = lambda r, t: (
    p0 - Qinj / (4 * hyd_cond * np.pi * wh) * expi(-(r**2) / (4.0 * alpha * t))
)

p_col = pressure(r_col, 5000.0)
# Elasticity matrix
...
insitu_tractions_global = np.full(
    (mesh.nelts, 3), [-tau_xz, -tau_yz, -sigma0]
...
k_sn = PropertyMap(
    np.zeros(mesh.nelts, dtype=int), np.array([[beta_spring * G, beta_spring * YoungM]])
...
eta_s = 1.0e-4 * 0.5 * G / c_s
eta_p = 1.0e-4 * 0.5 * G / c_p
...
mech_model = MechanicalModel(
    h1, mesh.nelts, frictionModel, precType="Jacobi", QDoperator=qd
)
...
model_parameters = {
    ...
    "Peak Friction": f_p,
    "Dilatancy peak": f_dil,
    "Dc": d_c,
    "T peak": T_p,
    "T residual": T_r,
    "Pre-stress ratio": S_factor,
    "Overpressure ratio": overpressure,
    "Peak to residual friction": f,
...
my_simul.setupSimulation(
    sol0, tend, dt=dt_ini, maxSteps=maxSteps, saveEveryNSteps=5, log_level="INFO"
)

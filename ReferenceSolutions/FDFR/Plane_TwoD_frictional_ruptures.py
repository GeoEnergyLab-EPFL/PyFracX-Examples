...
def criticallyStressed_lambda(T):
    ...
    return 2.0 / (np.pi ** (3 / 2)) * 1 / T  # O(1/\lambda^3)
...
def criticallyStressed_slip_Outer(xb):
    ...
    slip = (2.0 / np.pi ** (3 / 2)) * (
        np.arctanh(np.sqrt(1.0 - xb**2)) - np.sqrt(1.0 - xb**2)
    )
    return slip
...
def f_func_small(xhat):
    ...
    f_small = (
        (2.0 / np.pi ** (3 / 2))
        * (xhat**2)
        * (np.log(1.0 / np.abs(xhat)) - np.euler_gamma / 2.0 + 3.0 / 2.0)
    )  # O(x^4 \log (x))
    return f_small
...
def f_func_large(xhat):
    ...
    f_large = (2.0 / np.pi ** (3 / 2)) * (
        np.log(np.abs(xhat)) + np.euler_gamma / 2.0 + 1
    )  # # O(1/x^2)
    return f_large
...
def f_func_raw(xhat):
    "xhat=x/sqrt(4\alpha t)"
    # the function f(x) = \int_0^x (1/pi) \int_{-\infty}^{+\infty} Erfc[|s|]/(x-s) ds dx
    if (np.abs(xhat) > 0.2) & (np.abs(xhat) < 10.0):
...
def criticallyStressed_slip0(lam):
    ...
    slip0 = (
        (1 / lam)
        * (2.0 / (np.pi ** (3 / 2)))
        * (np.log(2.0 * lam) + np.euler_gamma / 2.0 - (1.0 / (4.0 * (lam**2))))
    )
    return slip0
...
def criticallyStressed_slip_Matched(xb, lam):
    ...
    slip_overlap = (2.0 / (np.pi) ** (3.0 / 2.0)) * (
        np.log(2.0 / np.abs(xb)) - 1.0 + (xb**2) / 4.0
    ) + (1 / (lam**2)) * (1.0 / (3.0 * (np.pi ** (3.0 / 2.0)))) * (
        1.0 / (xb**2) - 3.0 / 2.0
    )
...
def center_slip_app(lam):
    ...
    return (
        ...
        * lam
        * lam
        / (1 + (lam**2) / (np.log(6.0 + 2 * lam) + np.euler_gamma / 2.0))
    )
...
def marginallyPressurized_slip(xb, lam):
    ...
    """
    slip = (2.0 * (lam**2) / (np.pi ** (3 / 2))) * (
        np.sqrt(1.0 - xb**2) - (xb**2) * np.arctanh(np.sqrt(1 - xb**2))
    )  # + O(lam^4)
    return slip

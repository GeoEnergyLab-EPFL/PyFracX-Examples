...
class FluidDrivenAseismicSlipDilatancy2D:
    ...
    def evalG2(xi, lambda_, epsilon):
        ...
        g = (1 + a * (erf(lambda_) - (1 + epsilon) * erf(lambda_ * xi))) / (
            1 + a * erf(lambda_)
        )
        return g * (xi <= 1)
    ...
    def evalT2(lambda_, epsilon):
        ...
        func = lambda z: FluidDrivenAseismicSlipDilatancy2D.evalG2(
            np.cos(z), lambda_, epsilon
        )
    ...
    def Tfunction2D(epsilon):
        ...
        results = np.array(
            [FluidDrivenAseismicSlipDilatancy2D.evalT2(l, epsilon) for l in lambda_]
        )
        ...
        FluidDrivenAseismicSlipDilatancy2D.lambda_min = (
            lambda_positive[0] if lambda_positive.size > 0 else None
        )  # set explicitly
        FluidDrivenAseismicSlipDilatancy2D.lambda_max = (
            lambda_positive[-1] if lambda_positive.size > 0 else None
        )
        ...
        if len(lambda_positive) > 1:
            ...
            interpolation_function = interp1d(
                lambda_positive, results_positive, kind="linear"
            )
...
class FluidDrivenAseismicSlipDilatancy3D:
    ...
    def evalT3(lambda_, epsilon):
        ...
        func = lambda z: FluidDrivenAseismicSlipDilatancy3D.evalG3(
            np.cos(z), lambda_, epsilon
        ) * np.cos(z)
    ...
    def Tfunction3D(epsilon):
        ...
        # Compute parameter T for each lambda
        results = np.array(
            [FluidDrivenAseismicSlipDilatancy3D.evalT3(l, epsilon) for l in lambda_]
        )
        if all(x > 0 for x in results):
            negative_index = len(results)
        else:
        ...
        # Set class attributes for bounds
        FluidDrivenAseismicSlipDilatancy3D.lambda_min = (
            lambda_positive[0] if lambda_positive.size > 0 else None
        )
        FluidDrivenAseismicSlipDilatancy3D.lambda_max = (
            lambda_positive[-1] if lambda_positive.size > 0 else None
        )

        if len(lambda_positive) > 1:
            interpolation_function = interp1d(
                lambda_positive, results_positive, kind="cubic"
            )
            return interpolation_function
        else:
            return None
    ...
    def evalLambda3(Tparameter, epsilon):
        ...
        if interpolation_function is None:
            raise ValueError(
                "Interpolation function could not be created; check the range of lambda or data."
            )
        ...
        def equation_to_solve(lambda_):
            if (
                lambda_ < FluidDrivenAseismicSlipDilatancy3D.lambda_min
                or lambda_ > FluidDrivenAseismicSlipDilatancy3D.lambda_max
            ):
                return np.inf  # Repelling fsolve from out-of-bounds values
            return interpolation_function(lambda_) - Tparameter
...
def plot_T_vs_lambda_2D():
    epsilon_values = [
        ...
        10.0,
        100.0,
    ]  # Different epsilon values for demonstration
    lambda_range = np.logspace(-2, 1, 100)  # Lambda range

    plt.figure(figsize=(10, 6))
    for epsilon in epsilon_values:
        T_values = [
            FluidDrivenAseismicSlipDilatancy2D.evalT2(l, epsilon) for l in lambda_range
        ]
...
def plot_T_vs_lambda_3D():
    ...
    for epsilon in epsilon_values:
        T_values = [
            FluidDrivenAseismicSlipDilatancy3D.evalT3(l, epsilon) for l in lambda_range
        ]
        plt.plot(lambda_range, T_values, label=f"Epsilon = {epsilon}")

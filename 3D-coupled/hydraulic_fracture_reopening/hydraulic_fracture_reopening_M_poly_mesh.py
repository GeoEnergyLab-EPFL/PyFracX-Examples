...
T_h = k * H
wh_o = (12 * T_h) ** (1.0 / 3)  # initial hydraulic width

alpha_h = ((wh_o**2) / 12) / (
    fluid_visc * cf
)  # this is just to compute the initial time-step

# ---- in-situ conditions
sigmap_o = 10e6  # effective normal stress
tau_xz = taubyfsigop * sigmap_o
tau_yz = 0.0
p0 = 1.0e6  # background pore pressure
...
kn = (1 - beta_skempton) / (
    beta_skempton * cf * wh_o
)  # normal spring , 5e2 * YoungM  #

###
Dbar = (1 - beta_skempton) * (1.0 / (cf * sigmap_o)) * (pcbysigop ** (-2.0 / 3))
...
k_sn = PropertyMap(
    np.zeros(mesh.nelts, dtype=int), np.array([[ks, kn]])
...
model_parameters = {
    ...
    "Flow": {
        ...
        "Hydraulic conductivity": T_h / fluid_visc,

...
plt.figure()
plt.plot(t_, R, "r", label="Analytical")
...
# %%
plt.figure()
plt.plot(tts, lam * np.ones(len(tts)), "-r", label="Analytical")
plt.plot(tts, rmax / np.sqrt(4 * alpha * tts), "ko", label="Numerical")
plt.semilogx()
plt.xlabel("Time (s)")
plt.ylabel(r"$\lambda$ (-)")
plt.legend()
...
plt.savefig(
    os.path.join(figure_dir, "normalized_slip_profile.png"),
    dpi=200,
    bbox_inches="tight",
)
# plt.show()
...
plt.legend()
plt.savefig(
    os.path.join(figure_dir, "slip_profile_verification.png"),
    dpi=200,
    bbox_inches="tight",
)
# plt.show()

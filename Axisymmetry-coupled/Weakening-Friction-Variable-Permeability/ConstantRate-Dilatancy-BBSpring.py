...
wh_o = (12.0 * Tav) ** (1 / 3)  # [m] hydraulic width of the fault, cubic law
k_f = wh_o**2 / 12.0  # permeability
...
C_wd = 6.9 * 1.0e-8  # [m^3/Pa] wellbore storage

# %% Injection rate history
QinjTimeData = np.array([[0, 1e-1], [1000, 0.0]])
...
elastic_m = Elasticity(
    kernel, elas_properties, max_leaf_size=32, eta=2.0, eps_aca=1.0e-6
)
# Hmat creation
...
in_situ_effective_tractions = np.full(
    (Nelts, 2), [-tauo, -sigmap_o]
...
flow_model = FlowModelFractureSegments_axis(
    me, cubicModel, the_inj, scalingR=scale_r_f, scalingX=1.0
)
...
sol0 = HMFSolution(
    ...
    res_flow=np.zeros(me.nnodes, dtype=float),
)
...
dt_ini = (
    h_x**2 / alpha_hydro
)  # setting the initial time-step to have something "moving"
tend = 1500
maxSteps = 5000
...
ts_options = TimeIntegration_options(
    max_attempts=10,
    dt_reduction_factor=4.0,
    max_dt_increase_factor=5,
    lte_goal=0.001,
    acceptance_a_tol=res_atol,
    minimum_dt=dt_ini / 1000.0,
    maximum_dt=1e3,
    stepper_opts=step_solve_options,
)

events = EnforceTimes(list_times)
events.setTimeStepAfterEvent(
    1e-2
)  # do a small time step after an event, modify if necessary

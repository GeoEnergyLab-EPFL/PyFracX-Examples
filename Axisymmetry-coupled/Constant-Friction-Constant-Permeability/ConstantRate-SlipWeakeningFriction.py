...
Simul_description = "AxiSymm - ct rate - linear weakening friction - coupled"
now = datetime.now()
...
d_c = 1.0e-4  # [m], critical slip distance
...
domain_size = 1.5 * np.max(
    [lam_r * np.sqrt(4 * alpha * t_end), np.sqrt(4 * alpha * t_end)]
)
...
my_simul.setAdditionalStoppingCriteria(
    lambda sol: sol.Nyielded > int(0.95 * mesh.nelts)
)

...
Simul_description = "Axi Symm - ct rate - ct friction - coupled simulation - shutin"
now = datetime.now()
...
sol0 = HMFSolution(
    ...
    res_flow=np.zeros_like(po_nodes),
)
...
def stepWrapper(solN: HMFSolution, dt: float) -> HMFSolution:
    if solN.time < t_shutin:
        solTnew = hmf_coupled_step(
            solN, dt, mech_model, flow_model1, step_solve_options
        )
    else:
        solTnew = hmf_coupled_step(
            solN, dt, mech_model, flow_model2, step_solve_options
        )
    return solTnew

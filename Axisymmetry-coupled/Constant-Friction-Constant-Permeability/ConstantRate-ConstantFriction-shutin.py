...
#
#
# Fluid injection at constant rate into a frictional fault in 3D (modelled as axisymmetric problem) solved in a fully coupled H-M solver.
# Coupled simulation with constant rate injection and constant friction with shut-in
...
#This is the Reference Solutions folder in the PyFracX-Examples repository. Append path if necessary.
...
the_inj1 = Injection(np.array([0.0, 0.0]), np.array([[0.0, Qinj]]), "Rate")
the_inj2 = Injection(np.array([0.0, 0.0]), np.array([[0.0, 0.0]]), "Rate")

#during injection
flow_model1 = FlowModelFractureSegments_axis(mesh, constPerm, the_inj1)
#after shut-in
flow_model2 = FlowModelFractureSegments_axis(mesh, constPerm, the_inj2)

# properties are constant, so we construct the FE matrix once and for all.
flow_model1.setConductivity(None)
flow_model1.setStorage(None)
flow_model2.setConductivity(None)
flow_model2.setStorage(None)
...
def stepWrapper(solN: HMFSolution, dt: float) -> HMFSolution:
    if solN.time<t_shutin:
        solTnew = hmf_coupled_step(solN, dt, mech_model, flow_model1, step_solve_options)
    else:
        solTnew = hmf_coupled_step(solN, dt, mech_model, flow_model2, step_solve_options)
    return solTnew
# %%
...
model_parameters = {
    ...
    "T parameter": T, "Shut-in time": t_shutin,
}
# %%
...
Simul_description = (
    "Axi Symm - ct rate - ct friction - coupled simulation - shutin"
)
now = datetime.now()
dt_string = now.strftime("%d-%m-%Y-%H:%M:%S")
basename = "AxiSymm-ctRate-ctFriction-coupled-shutin"
...
events = EnforceTimes(np.array([t_shutin]))

events.setTimeStepAfterEvent(new_dt) 
...
ax.plot(
    r_col,
    complete_slip_profile_cs(G, f_p, dp, alpha, T, tts[-1], lambda_analytical, r_col),
    "-k",
    ms=1.2,
    label="Analytical",
)

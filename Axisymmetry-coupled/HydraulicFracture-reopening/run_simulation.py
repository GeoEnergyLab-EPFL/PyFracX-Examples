...
pcbysigop = 1.2e6 # p_c / \sigma_o^p @INPUT
...
elastic_m = Elasticity(
    kernel, elas_properties, max_leaf_size=100, eta=3.0, eps_aca=1.0e-4
)
# hmat creation
...
# max 1 kPa, norm(t0)
res_atol = 5e-4 * max(np.linalg.norm(effective_traction.flatten()), 1e3 * mesh.nelts)
...
# dd/mm/YY H:M:S
folder = home + "/data/" # @INPUT, where to save the data
Simul_description = "3D Axis Symm - constant rate - zero toughness HF"
now = datetime.now()
...
dt_ini = min(0.1 * h_r**2 / alpha, 0.1)
print("dt_ini", dt_ini)
